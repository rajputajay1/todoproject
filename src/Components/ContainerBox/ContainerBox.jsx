...
const ContainerBox = ({ name, img, data }) => {
  ...
  const [deletepopup, setDeletePopup] = useState(false);
  const [sharepopupTaskId, setSharePopupTaskId] = useState(null); // State to hold the task ID for which share popup is open
  const [taskToDelete, setTaskToDelete] = useState(null);
  ...
  const handleAllCollapse = () => {
    setCollapsedTasks(data.map(task => task._id));
  };
